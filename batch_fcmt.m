function [mout xyz M L]=batch_fcmt(fsave,Ln)
...
% % Epicentre coords (This is used in SINGLE CMT El Mayor)
% lonepi=-117.6:0.2:-113.6;
% latepi=30.4:0.2:34.4;
% depthepi=3e3:3e3:27e3;
% azi=0;

% %Epicentre coords (This is used in SOPAC gridsearch Toki)
% lonepi=143.2:0.2:144.8;test
...
% lonepi=143.0661;  %(This is used in finite synthetic test)
% latepi=37.9603;
% depthepi=30e3;
% azi=13;

% %Epicentre coords (This is used in SOPAC Inidividual Toki)
% lonepi=144;
% latepi=42.2;
% depthepi=30e3;
% %Line griding
...
data='rapid_coseis_kalgps_tohoku';
...
wdflag=0; %Weigh flag by distance to centroid (1/d^2)
dcflag=5; %Force double couple? 5test
pflag=0; %Plot data for each epoch and each node
multiflag=1;  %Multiple CMT inversion?
...
cd(mainpath);
stas=[coseis.lon coseis.lat zeros(size(coseis.lon))];
save('stations.xy','stas','-ascii');
cd(path);
...
%         [longrid latgrid zgrid inode]=getgrid(lonall(k),latall(k),depthall(k),aziall(k),linelength,nsources,grid);
        ...
        for j=1:nnode
            Gtemp=load([path '/denseGF/' fgreen num2str(inode(j))]);
            G(:,:,j)=Gtemp(jnan,:);
        end
