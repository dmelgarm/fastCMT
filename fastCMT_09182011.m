function [m L synthetics]=fastCMT(coseis,G,velmod,epi,psmecaf,weightflag,plotflag,dweight,Ln)
...
for k=263:nt
    ...
    if nsta>2
        ...
        ds=zeros(nsta*3,1);
        dx=Ncurrent;
        dy=Ecurrent;      %Un-normalized for moment
        dz=Ucurrent;
        azi=az(c);
        d=ones(size(c,1)*3,1);
        iinv=1:1:nsta*3;
        ...
        dobs=d;
        dsyn=Gj*mt; 
        L(k)=(1-(sqrt(sum((dobs-dsyn).^2)))/(sqrt(sum(dobs.^2))))*100;
        ...
        synthetics(s:Ns,9)=lon(c);
        synthetics(s:Ns,10)=lat(c);
